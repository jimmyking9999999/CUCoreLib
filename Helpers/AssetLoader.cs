...
public static class AssetLoader
{
    ...
    internal static Dictionary<string, AudioClip> AudioClipCache = new();

    internal static Dictionary<string, RegisteredSpriteAnimation> SpriteAnimationCache =
        new(StringComparer.OrdinalIgnoreCase);

    private static readonly Dictionary<string, string[]> ResourceNameCache = new(StringComparer.OrdinalIgnoreCase);

    // Prevent repeated error spam for the same missing asset lookup
    private static readonly HashSet<string> LoggedMissingResources = new(StringComparer.OrdinalIgnoreCase);
    ...
    private static string[] GetManifestResourceNames(Assembly sourceAssembly)
    {
        ...
        var cacheKey = sourceAssembly.FullName;
